...
while (<IN>) {
  ...
  print OUT $arr[0]."\t".sprintf("%.5f",$arr[1]*$factor)."\t".sprintf("%.5f",$arr[2]*$factor)."\t".sprintf("%.5f",$arr[3]*$factor)."\t".sprintf("%.5f",$arr[4]*$factor)."\t".sprintf("%.5f",$arr[5]*$factor)."\t".sprintf("%.5f",$arr[6]*$factor)."\t".sprintf("%.5f",$arr[7]*$factor)."\n";
}
close(IN);
close(OUT);
print "\nTotal number of reactions in Expression File\t".$count."\n";
