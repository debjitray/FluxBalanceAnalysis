% THIS CODE IS FOR MODEL 2 with the MicroArray Data
...
% Experimental Data Reader for 8 Time Frames (Need to change this)
Exp_Data = importdata('/Users/debjit/Documents/Ymet/MyModel/SubNetwork/Model2/1.Protein Data Preparation/Experimental Data/5.Experimental Data_MAD.txt');   % Use Exp_Data.data for using the data part of it, ROW1 corresponds to First TF
TF_Exp = Exp_Data.data;


% Expression Data Reader for 8 Time Frames
Protein_Data = importdata('/Users/debjit/Documents/Ymet/MyModel/SubNetwork/Model2/1A.MicroArrayDataPreparation/8.62Finale_Expression.txt');   % Use Protein_Data.data for using the data part of it, COLUMN1 corresponds to First TF
...
for k=1:10     
  ...
  %------------------------------------------------------------------------------------------------------------------------------------------------------%
            ...
            % OPTIMIZING THE MODEL'
             FBAmax=optimizeCbModel(finalModel,'min');
             Z=FBAmax.f
             zValues(k,m)=Z;
             ...
             [MIN_FVA,MAX_FVA]=fluxVariability(finalModel,100,'min');
...
% ******************  PEARSON AND SPEARMAN CORRELATION CALCULATIONS  ******************
   ...
   S_Time1=0;
        ...
        Spearman(k,m)=S_Time1;
    end;
    
end;


dlmwrite('Results_M2_MAD/1A.Average_Pathway_Optimal.csv',AverageLOPT);
dlmwrite('Results_M2_MAD/1B.Average_Pathway_MIN.csv', AverageMIN_8Pathway);
dlmwrite('Results_M2_MAD/1C.Average_Pathway_MAX.csv', AverageMAX_8Pathway);

dlmwrite('Results_M2_MAD/2A.Individual_8Pathway_Optimal.csv',Individual_LOPT);
dlmwrite('Results_M2_MAD/2B.Individual_8Pathway_MIN.csv',MIN_8Pathway);
dlmwrite('Results_M2_MAD/2C.Individual_8Pathway_MAX.csv',MAX_8Pathway);

dlmwrite('Results_M2_MAD/3A.Individual_All_Optimal.csv',FluxLOPT);
dlmwrite('Results_M2_MAD/3B.Individual_All_MIN.csv',FVA_MIN_All);
dlmwrite('Results_M2_MAD/3C.Individual_All_MAX.csv',FVA_MAX_All);


fidZ = fopen('Results_M2_MAD/0.Z Values.txt','wt');
fprintf(fidZ, [repmat('%.5f\t', 1, size(zValues,2)), '\n'], zValues');
fclose(fidZ);

fidE = fopen('Results_M2_MAD/Euclidean.txt','wt');
fprintf(fidE, [repmat('%.5f\t', 1, size(Euclidean,2)), '\n'], Euclidean');
fclose(fidE);

fidP = fopen('Results_M2_MAD/Pearson.txt','wt');
fprintf(fidP, [repmat('%.5f\t', 1, size(Pearson,2)), '\n'], Pearson');
fclose(fidP);

fidS = fopen('Results_M2_MAD/Spearman.txt','wt');
fprintf(fidS, [repmat('%.5f\t', 1, size(Spearman,2)), '\n'], Spearman');
fclose(fidS);
