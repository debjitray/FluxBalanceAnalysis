...
Exp_Data = importdata('1.Protein Data Preparation/Experimental Data/5.Experimental Data.txt');   % Use Exp_Data.data for using the data part of it, ROW1 corresponds to First TF
TF_Exp = Exp_Data.data;


% Expression Data Reader for 18 Time Frames
Protein_Data = importdata('1.Protein Data Preparation/8.62Finale_Expression.txt');   % Use Protein_Data.data for using the data part of it, COLUMN1 corresponds to First TF
...
for k=1:10     
  ...
  %------------------------------------------------------------------------------------------------------------------------------------------------------%
            ...
            % OPTIMIZING THE MODEL'
             [v,z,exitflag1,output1,lambda1]=linprog(-finalModel.c,[],[],finalModel.S,finalModel.b,finalModel.lb,finalModel.ub); % REMEMBER TO CHANGE THE 'MAX'  in LINE 89
             Z=z
             zValues(k,m)=-z;
...
fidZ = fopen('Results_M2/0.Z Values.txt','wt');
fprintf(fidZ, [repmat('%.5f\t', 1, size(zValues,2)), '\n'], zValues');
fclose(fidZ);

fidE = fopen('Results_M2/Euclidean.txt','wt');
fprintf(fidE, [repmat('%.5f\t', 1, size(Euclidean,2)), '\n'], Euclidean');
fclose(fidE);

fidP = fopen('Results_M2/Pearson.txt','wt');
fprintf(fidP, [repmat('%.5f\t', 1, size(Pearson,2)), '\n'], Pearson');
fclose(fidP);

fidS = fopen('Results_M2/Spearman.txt','wt');
